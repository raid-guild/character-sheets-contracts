  ...
  UUPSUpgradeable
{
  ...
  function initialize(bytes calldata _encodedParameters) external initializer {
    ...
    (clonesStorage, implementationStorage, metadataURI, baseTokenURI) = abi
      .decode(_encodedParameters, (address, address, string, string));
    clones = IClonesAddressStorage(clonesStorage);
    erc6551Registry = IImplementationAddressStorage(implementationStorage)
      .erc6551Registry();
    erc6551CharacterAccount = IImplementationAddressStorage(
  ...
  function rollCharacterSheet(
    ...
    if (
      existingCharacterId != 0 ||
      _sheets[existingCharacterId].playerAddress == player
    ) {
      // must restore sheet
   ...
   */
  function unequipItemFromCharacter(
    uint256 characterId,
    uint256 itemId
  ) external onlyCharacter {
  ...
  function restoreSheet() external returns (address) {
    ...
    if (
      ...
      !ICharacterEligibilityAdaptor(clones.characterEligibilityAdaptor())
  ...
  function updateErc6551CharacterAccount(
    address newERC6551CharacterAccount
  ) public onlyAdmin {
  ...
  function getCharacterSheetByCharacterId(
    uint256 characterId
  ) public view returns (CharacterSheet memory) {
    if (characterId == 0 || characterId > totalSheets) {
      revert Errors.CharacterError();
    }
    return _sheets[characterId];
  }

  function getCharacterIdByAccountAddress(
    address _account
  ) public view returns (uint256 id) {
    uint256 characterId = _characterSheets[_account];
    if (characterId == 0) {
      revert Errors.CharacterError();
    }
    return characterId;
  ...
  function getCharacterIdByPlayerAddress(
    address _player
  ...
  function _checkRollReverts(address player) internal view {
    if (
      erc6551CharacterAccount == address(0) || erc6551Registry == address(0)
    ) {
